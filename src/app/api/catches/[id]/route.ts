...
import { newCatchPatchSchema } from "@/lib/schemas"
import {
...
export async function GET(_: Request, { params }: Ctx) {
  try {
    ...
    const item = await prisma.newCatch.findUnique({
      where: { id },
...
export async function PATCH(req: Request, { params }: Ctx) {
  try {
    ...
    const data = newCatchPatchSchema.parse(body)
    const item = await prisma.newCatch.update({
      where: { id },
      data,
      include: { breed: { include: { species: true } } },
    })
...
export async function DELETE(_: Request, { params }: Ctx) {
  try {
    ...
    const { id } = await params
    await prisma.newCatch.delete({ where: { id } })
